...
// VIẾT LẠI theo ĐÚNG logic LCD (lcd_user.c → page_scan_detail)
//
// Quy ước index từ ESP32 (scan_device.c → analyse_scan_result):
//   fp1 = index LỚN NHẤT mà Port1 còn thấy   (-1 nếu Port1 không thấy ai)
//   fp2 = index NHỎ NHẤT mà Port2 còn thấy    (= count nếu Port2 không thấy ai)
//   → Port1 phủ index [0 .. fp1]
//   → Port2 phủ index [fp2 .. count-1]
//   → vùng cô lập (mất do đứt dây) = index [fp1+1 .. fp2-1]
//
// Quy ước segment (count+1 đoạn dây):
//   seg 0      = Port1 ↔ ID[0]
//   seg i      = ID[i-1] ↔ ID[i]
//   seg count  = ID[count-1] ↔ Port2
//
// 4 loại trạng thái:
//   "normal"         — dây nguyên, tất cả thiết bị online
//   "device_offline" — dây nguyên nhưng có thiết bị không phản hồi (lỗi thiết bị)
//   "single"         — dây đứt 1 chỗ
//   "double"         — dây đứt 2 chỗ (thiết bị giữa bị cô lập)
// ─────────────────────────────────────────────────────────────────────────────
function analyzeWire(scan, detail) {
  ...
  // ── Loại 1: Dây nguyên (1 port thấy đủ), tất cả online ──
  if ((wire_p1_ok || wire_p2_ok) && !hasOffline)
    return { type: "normal", breaks: [], gapSet: new Set(), originalId, fp1, fp2, count };

  // ── Loại 2: Dây nguyên nhưng có thiết bị offline → LỖI THIẾT BỊ, không vẽ break ──
  if (wire_p1_ok || wire_p2_ok)
    return { type: "device_offline", breaks: [], gapSet: new Set(), originalId, fp1, fp2, count };

  // ── Loại 3: Cả 2 port đều KHÔNG thấy đủ → DÂY ĐỨT ──
  // Tính 2 ranh giới đứt — y hệt cách LCD suy ra điểm đứt từ fp1/fp2
  //   leftSeg  : ranh giới đứt phía Port1 (cuối vùng Port1 phủ)
  //   rightSeg : ranh giới đứt phía Port2 (đầu vùng Port2 phủ)
  const leftSeg  = (fp1 === -1)    ? 0     : fp1 + 1;
  const rightSeg = (fp2 === count) ? count : fp2;

  // Mô tả 1 điểm đứt theo segment index
  const segDesc = (seg) => {
  ...
  const breaks = [];
  if (leftSeg === rightSeg) {
    // ĐỨT 1 CHỖ — 2 ranh giới trùng nhau (không có thiết bị bị kẹt giữa)
    breaks.push({ segIndex: leftSeg, desc: segDesc(leftSeg) });
  } else {
    // ĐỨT 2 CHỖ — luôn vẽ ĐỦ 2 điểm đứt, kể cả khi chỉ 1 thiết bị bị kẹt giữa
    // (đây chính là bug cũ: thiếu điểm đứt thứ 2 khi gap chỉ có 1 thiết bị)
    breaks.push({ segIndex: leftSeg,  desc: segDesc(leftSeg) });
    breaks.push({ segIndex: rightSeg, desc: segDesc(rightSeg) });
  }

  // Thiết bị nằm trong vùng cô lập [leftSeg .. rightSeg-1]
  // → mất kết nối DO ĐỨT DÂY, KHÔNG phải thiết bị tự tắt
  const gapSet = new Set();
  for (let d = leftSeg; d < rightSeg; d++) {
    if (d >= 0 && d < count) gapSet.add(originalId[d]);
  }

  const type = breaks.length === 1 ? "single" : "double";
  return { type, breaks, gapSet, originalId, fp1, fp2, count };
}
...
function getDescription(analysis) {
  ...
  if (type === "device_offline")
    return { text: "⚠️ Cable is normal but some devices are not responding. Check device power or Modbus configuration.", color: COLOR.warn };

  if (type === "single")
    return { text: `⚠️ Single break at: ${breaks[0].desc}. The other port is used as fallback master.`, color: COLOR.warn };

  if (type === "double")
    return { text: `🔴 Two breaks — ${breaks[0].desc} | ${breaks[1].desc}. Device(s) between the breaks are isolated by the cable fault.`, color: COLOR.err };
  ...
  return (
    <Box sx={{ overflowX: "auto" }}>
      <svg width={SVG_W} height={130} style={{ display: "block", minWidth: SVG_W }}>
        ...
        {/* Các thiết bị */}
        {originalId.map((id, i) => {
          const isGap  = gapSet.has(id);                       // mất do đứt dây
          const active = statusMap[id] === "active" && !isGap; // online thật
          const x = mXs[i];
          const c = active ? COLOR.ok : COLOR.err;
          // Nhãn phân biệt: online / no link (đứt dây cô lập) / offline (thiết bị tắt)
          const label = active ? "online" : (isGap ? "no link" : "offline");
          return (
            <g key={id}>
              ...
              <text x={x + NW / 2} y={Y + NH / 2 + 15} textAnchor="middle" fontSize="9"  fill={c}>{label}</text>
            </g>
          );
        })}
...
export default function ScanDetailModal({ open, onClose, scan, detail, loading }) {
  ...
  // Phân loại thiết bị mất: "no link" (đứt dây) vs "offline" (thiết bị tắt)
  const allInactive = (detail?.devices || []).filter(d => d.status === "inactive").map(d => d.modbus_id);
      ...
      <DialogContent sx={{ pt: 2 }}>
        ...
        <Box sx={{ mt: 1.5, display: "flex", gap: 2, flexWrap: "wrap" }}>
          {[
            ...
            { color: COLOR.err, label: "Lost device",    type: "box"  },
        ...
        <Box sx={{ display: "flex", gap: 1.5, flexWrap: "wrap",
                   mb: (noLinkIds.length || offlineIds.length) > 0 ? 1.5 : 0 }}>
        ...
        {/* Thiết bị mất do ĐỨT DÂY (no link) */}
        {!loading && noLinkIds.length > 0 && (
          <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", alignItems: "center", mb: 1 }}>
            <Typography fontSize={12} color={COLOR.muted}>Isolated by cable break:</Typography>
            {noLinkIds.map(id => (
